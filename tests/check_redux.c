...
START_TEST(test_contig_simple) {
  ...
  static const uint32_t data1[9] = {1, 2, 3, 4, 5, 6, 7, 8, 9};
  uint32_t data2[3] = {0};
  ...
  ga_assert_ok(GpuArray_empty(&in, ctx, GA_UINT, 2, dims, GA_C_ORDER));
  ga_assert_ok(GpuArray_write(&in, data1, sizeof(data1)));

  ga_assert_ok(GpuArray_empty(&out, ctx, GA_UINT, 1, dims, GA_C_ORDER));

  ga_assert_ok(GpuReduction_new(&gr, ctx, NULL, "a + b", "0", GA_UINT, 2, 0));

  ck_assert_ptr_ne(gr, NULL);

  ga_assert_ok(GpuReduction_call(gr, &in, 1, &out));

  ga_assert_ok(GpuArray_read(data2, sizeof(data2), &out));

  ck_assert_int_eq(data2[0], 5);
  ck_assert_int_eq(data2[1], 7);
  ck_assert_int_eq(data2[2], 9);

  ga_assert_ok(GpuReduction_call(gr, &in, 2, &out));
