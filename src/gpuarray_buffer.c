...
gpucontext *gpucontext_init(const char *name, int dev, int flags, int *ret) {
  gpucontext *res;
  const gpuarray_buffer_ops *ops = gpuarray_get_ops(name);
  if (ops == NULL) FAIL(NULL, GA_INVALID_ERROR);
  res = ops->buffer_init(dev, flags, ret);
  if (res == NULL) return NULL;
  res->ops = ops;
  if (gpucontext_property(res, GA_CTX_PROP_BLAS_OPS, &res->blas_ops) !=
      GA_NO_ERROR)
    res->blas_ops = NULL;
  res->blas_handle = NULL;
  if (gpucontext_property(res, GA_CTX_PROP_COMM_OPS, &res->comm_ops) !=
      GA_NO_ERROR)
    res->comm_ops = NULL;
...
void gpucontext_deref(gpucontext *ctx) {
  if (ctx->blas_handle != NULL) ctx->blas_ops->teardown(ctx);
...
const char *gpucontext_error(gpucontext *ctx, int err) {
  if (ctx != NULL) {
    switch (err) {
    case GA_IMPL_ERROR: return ctx->ops->ctx_error(ctx);
    case GA_BLAS_ERROR: return gpublas_error(ctx);
    case GA_COMM_ERROR: return gpucomm_error(ctx);
    }
  }
...
int gpudata_move(gpudata *dst, size_t dstoff, gpudata *src, size_t srcoff,
                 size_t sz) {
  return ((partial_gpudata *)src)
      ->ctx->ops->buffer_move(dst, dstoff, src, srcoff, sz);
}
...
int gpudata_write(gpudata *dst, size_t dstoff, const void *src, size_t sz) {
  return ((partial_gpudata *)dst)->ctx->ops->buffer_write(dst, dstoff, src, sz);
}
...
int gpudata_property(gpudata *b, int prop_id, void *res) {
  return ((partial_gpudata *)b)
      ->ctx->ops->property(NULL, b, NULL, prop_id, res);
}
...
int gpukernel_call(gpukernel *k, unsigned int n, const size_t *ls,
  ...
  return ((partial_gpukernel *)k)
      ->ctx->ops->kernel_call(k, n, ls, gs, shared, args);
}
...
int gpukernel_property(gpukernel *k, int prop_id, void *res) {
  return ((partial_gpukernel *)k)
      ->ctx->ops->property(NULL, NULL, k, prop_id, res);
}
