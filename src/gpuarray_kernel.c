...
int GpuKernel_init(GpuKernel *k, gpucontext *ctx, unsigned int count,
  ...
  if (k->args == NULL) return GA_MEMORY_ERROR;
  k->k = gpukernel_init(ctx, count, strs, lens, name, argcount, types, flags,
                        &res, err_str);
  if (res != GA_NO_ERROR) GpuKernel_clear(k);
  return res;
}

void GpuKernel_clear(GpuKernel *k) {
  if (k->k) gpukernel_release(k->k);
  free(k->args);
  k->k = NULL;
  k->args = NULL;
}
...
int GpuKernel_sched(GpuKernel *k, size_t n, size_t *ls, size_t *gs) {
  ...
  if (target_g > max_g) target_g = max_g;
  target_l = 512;
  if (target_l > max_l) target_l = max_l;
...
int GpuKernel_call(GpuKernel *k, unsigned int n, const size_t *bs,
