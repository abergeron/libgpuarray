...
static inline int check_gpuarrays(int times_src, const GpuArray *src,
  ...
  if (times_src * count_src != times_dest * count_dest) return GA_VALUE_ERROR;
  if (src->typecode != dest->typecode) return GA_VALUE_ERROR;
...
int GpuArray_reduce_from(const GpuArray *src, int opcode, int root,
                         gpucomm *comm) {
  size_t total_elems;
  if (!GpuArray_ISALIGNED(src)) return GA_UNALIGNED_ERROR;
...
int GpuArray_broadcast(GpuArray *array, int root, gpucomm *comm) {
  ...
  GA_CHECK(gpucomm_get_rank(comm, &rank));
  if (rank == root) {
    if (!GpuArray_CHKFLAGS(array, GA_BEHAVED)) return GA_UNALIGNED_ERROR;
  } else {
    if (!GpuArray_ISALIGNED(array)) return GA_UNALIGNED_ERROR;
  }
